...
const Leaderboards: React.FC = () => {
  ...
  const handleScroll = () => {
    ...
    // Calculate menu opacity
    const opacity = 1;
    setMenuOpacity(opacity);

    // Hide menu when scrolling near the bottom of the page
    const windowHeight = window.innerHeight;
    const pageHeight = document.body.scrollHeight - window.innerHeight;
    if (opacity === 1 && window.scrollY > pageHeight * 0.9) {
      setMenuOpacity(0);
    }

    // Calculate offsets for each section with a margin
    const margin = 150;
  ...
  return (
    <>
      ...
      {/* <Helmet>
        <title>Leaderboards | ABLUVA</title>
        <meta name="description" content="Explore ABLUVA's achievements on various leaderboards. Check the performance of our models across different datasets and tasks." />
      </Helmet> */}

      <div className=''>
        <p className="text-4xl md:text-5xl primary-color-g pt-20 pb-10 text-center" id="Leaderboards" style={{ scrollMarginTop: '140px' }}>Leaderboards</p>

        {/* Menu section */}
{/*       
        <div className='flex items-center justify-center research-menu pb-20' style={{ opacity: menuOpacity }}>
